...
async def get_all_sports(page: zd.Tab) -> list:
...
async def get_all_bets(page: zd.Tab):
    event = {}
...
async def get_event_bets(url: str, browser: zd.Browser, semaphore: asyncio.Semaphore) -> list:
...
async def get_all_events_bets(events: list, browser: zd.Browser) -> list:
...
async def get_league_data(page: zd.Tab, sport: str, data: dict = None) -> dict:
...
async def main():
    ...
    browser = await zd.start(browser_args=['--start-maximized'])
...
if __name__ == "__main__":
    duration = timeit.timeit(lambda: zd.loop().run_until_complete(main()), number=1)
    print(duration)
